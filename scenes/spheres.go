...
func main() {
	...
	samples := flag.Int("samples", 300, "Average per pixel samples to take.")
	heat := flag.String("heat", "", "Heatmap png filename.")
	flag.Parse()

	scene := trace.Scene{}
	camera := trace.NewCamera(960, 540, 0.045)
	...
	scene.Add(trace.Sphere{trace.Vector3{0.25, 0, -2}, 0.1, redPlastic})
	scene.Add(trace.Sphere{trace.Vector3{0, 0, -0.7}, 0.1, silver})
	scene.Add(trace.Sphere{trace.Vector3{0.3, 0, -0.5}, 0.1, gold})
	scene.Add(trace.Sphere{trace.Vector3{-0.2, 0, -2.0}, 0.1, frostedGlass})
	scene.Add(trace.Sphere{trace.Vector3{-0.7, 0, -5.0}, 0.1, bluePlastic})
	scene.Add(trace.Sphere{trace.Vector3{-1, 0, -4.0}, 0.1, silver})
	scene.Add(trace.Sphere{trace.Vector3{15.0, 25.0, -10.0}, 15.0, light})
	scene.Add(trace.Sphere{trace.Vector3{0, -10000.1, -7}, 10000, whitePlastic})
	camera.Move(0, 0.1, 0.3)
	camera.LookAt(0, 0, -0.7)
	camera.Focus(0, 0, -0.68, 8)
