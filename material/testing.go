...
func (t Testing) Eval(wi, wo geom.Direction) rgb.Energy {
	...
	F := fresnelSchlick(wo, geom.Up, t.F0.Mean())
	if t.u < F {
		D := ggx(wi, wo, wg, t.Roughness)  // The NDF (Normal Distribution Function)
		G := smithGGX(wo, wg, t.Roughness) // The Geometric Shadowing function
		r := (F * D * G) / (4 * wg.Dot(wi) * wg.Dot(wo))
		return t.F0.Scaled(r)
	}
	return rgb.Energy{0, 0, 0}
}
